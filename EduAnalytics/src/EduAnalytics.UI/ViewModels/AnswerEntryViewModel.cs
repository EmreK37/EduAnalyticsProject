...
public partial class AnswerEntryViewModel : ObservableObject
{
    ...
    private void CopyToClipboard()
    {
        try
        {
            ...
            {
                sb.Append($"{student.StudentNumber}\t{student.FullName}");
    ...
    private void PasteFromClipboard()
    {
        try
        {
            ...
            for (int i = startIdx; i < lines.Length; i++)
            {
                ...
                for (int c = 2, qIdx = 0; c < cols.Length && qIdx < studentRow.Cells.Count; c++, qIdx++)
                {
    ...
    private void DownloadExcel()
    {
        try
        {
            ...
            if (sfd.ShowDialog() == true)
            {
                ...
                {
                    ws.Cell(1, i + 3).Value = $"Soru {Questions[i].QuestionNumber}";
                }
    ...
    public async Task LoadAsync(int examId)
    {
        ...
        try
        {
            ...
            // Öğrenci satırlarını oluştur, hücreleri soru sırasına göre aç
    ...
    private async Task SaveAsync()
    {
        ...
        try
        {
            var updates = StudentRows
                ...
                .Select(c => new StudentAnswerUpdate
                {
